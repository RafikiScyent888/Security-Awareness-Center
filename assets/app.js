...
import { questionHints, lookTarget, LOOK_LABEL } from "./hints.js";
...
/* ---------------- questions ---------------- */
/* =====================================================================
   Guided hints, and what to look at when they run out

   Silent for the first two wrong answers. From the third, one rung per
   attempt. When the ladder is spent the page marks the evidence in the
   question's own lens and the control to change, and clears the field on
   request — the commonest reason a stuck student stays stuck is their own
   wrong pick sitting in the box they are staring at.

   The mark never names the answer. It says where to look and where to act.
   ===================================================================== */
let litNodes = [];
...
/* The evidence lives in the question's OWN tile, so it is found rather than
   listed. A lens added later is covered the day it is written instead of
   falling silently out of the feature. */
const EVIDENCE = ".logwrap, .fw, .matrix, .picks, .remwork, .gloss";
function highlight(card, where) {
  clearHighlight();
  const tile = card.closest(".tile");
  const marks = (where === "evidence" && tile) ? [...tile.querySelectorAll(EVIDENCE)] : [];
  marks.forEach((n, i) => {
    n.classList.add("lit");
...
function hintBox(host, opts) {
  ...
  let wrong = 0;
  const wipe = () => { wrong = 0; box.hidden = true; box.innerHTML = ""; };
  return {
    ...
    wrong: (list) => {
      ...
      const where = opts.where || "evidence";
      const marked = opts.onLight ? opts.onLight(where) : 0;
      box.appendChild(el("span", "hint__t hint__t--last",
        (where === "none" || !marked) ? LOOK_LABEL.none : LOOK_LABEL.evidence));
      if (opts.onReset) {
        const again = el("button", "btn btn--again", "Clear my answer and try again");
        again.type = "button";
        again.addEventListener("click", () => {
          opts.onReset();
          /* Hints and wrong-count both survive: nothing earned is taken away,
             and nobody can farm a fresh ladder by failing on purpose. */
          again.disabled = true;
...
function renderQuestions(host, qs, onGraded) {
  ...
  qs.forEach((q) => {
    ...
    function check() {
      ...
      if (ok) hints.right();
      else hints.wrong(q.hints ? q.hints() : questionHints(q.id));
      updateScore();
