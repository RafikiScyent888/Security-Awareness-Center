...
function renderQuestions(host, qs, onGraded) {
  ...
  qs.forEach((q) => {
    ...
    // Switching instructor mode back off has to take the answers away again,
    // or a class watches the screen go from graded to ungraded with the key
    // still sitting there. Anything the student earned themselves stays.
    q._unreveal = () => {
      if (fb.dataset.revealed !== "1") return;
      delete fb.dataset.revealed;
